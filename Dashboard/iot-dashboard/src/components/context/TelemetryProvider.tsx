...
interface TelemetryContextValue {
    ...
    humid: SensorData[]
    soil: SensorData[]
    predictedTemp: SensorData[]
    predictedHumid: SensorData[]
    predictedSoil: SensorData[]
    pH: SensorData[]
    ORP: SensorData[]
    TUR: SensorData[]
    EC: SensorData[]
...
export function TelemetryProvider({
    ...
    const [temp, setTemp] = useState<SensorData[]>([])
    const [humid, setHumid] = useState<SensorData[]>([])
    const [soil, setSoil] = useState<SensorData[]>([])
    const [predictedTemp, setPredictedTemp] = useState<SensorData[]>([])
    const [predictedHumid, setPredictedHumid] = useState<SensorData[]>([])
    const [predictedSoil, setPredictedSoil] = useState<SensorData[]>([])
            ...
            try {
                ...
                webSocket.onmessage = async (event) => {
                    ...
                    if (data?.temperature?.length > 0) {
                        setTemp((curData) => [...curData, { time: data.temperature[0][0], value: data.temperature[0][1] }])
                    }
                    if (data?.humidity?.length > 0) {
                        setHumid((curData) => [...curData, { time: data.humidity[0][0], value: data.humidity[0][1] }])
                    }
                    if (data?.soil_moisture?.length > 0) {
                        setSoil((curData) => [...curData, { time: data.soil_moisture[0][0], value: data.soil_moisture[0][1] }])
                    }
                    if (data?.predicted_temp?.length > 0) {
                        setPredictedTemp((curData) => [...curData, { time: data.predicted_temp[0][0], value: data.predicted_temp[0][1] }])
                    }
                    if (data?.predicted_humid?.length > 0) {
                        setPredictedHumid((curData) => [...curData, { time: data.predicted_humid[0][0], value: data.predicted_humid[0][1] }])
                    }
                    if (data?.predicted_soil?.length > 0) {
                        setPredictedSoil((curData) => [...curData, { time: data.predicted_soil[0][0], value: data.predicted_soil[0][1] }])
                    }
    ...
    return (
        <TelemetryContext.Provider value={{ temp, humid, soil, predictedTemp, predictedHumid, predictedSoil, pH, ORP, TUR, EC, predictedPH, predictedORP, predictedTUR, predictedEC, currentFWVersion, fwVersion, fwTitle, executed_cronjob, name }}>
            {children}
