...
#include "./Sensors/include/readTempHumi.h"
#include "./Sensors/include/readSoilMoisture.h"
...
void publisherTask(void* parameter) {
    ...
    while (1) 
    {
        ...
        if (client.connected()) {
            ...
            float predicted_humi = 0.0;
            float predicted_soil = 0.0;

            std::vector<float> prediction = processModel(temperature, humidity, soilMoisture);
            
            if (prediction.size() == 3) {
                predicted_temp = prediction[0];
                predicted_humi = prediction[1];
                predicted_soil = prediction[2];
            }

            char payload[256];
            sprintf(payload, "{\"temperature\": %.2f, \"predicted_temperature\": %.2f, \"humidity\": %.2f, \"predicted_humidity\": %.2f, \"soil_moisture\": %.2f, \"predicted_soil_moisture\": %.2f, \"long\": %.6f, \"lat\": %.6f}",
                temperature, predicted_temp, humidity, predicted_humi, soilMoisture, predicted_soil, longitude, latitude);
